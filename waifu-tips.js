    ...
    class s {
        ...
        async loadModel(t, s, n) {
            if (localStorage.setItem("modelId", t), localStorage.setItem("modelTexturesId", s), o(n, 4e3, 10), this.useCDN) {
                ...
                const textures = this.modelList.models[t];
                let textureName;
                let textureIndex = parseInt(s) || 0;
        ...
        async loadRandModel() {
            ...
            let s = parseInt(localStorage.getItem("modelTexturesId")) || 0;
            if (this.useCDN) {
                ...
                if (Array.isArray(textures)) {
                    s = (s + 1) >= textures.length ? 0 : (s + 1);
                    nextTexture = textures[s];
                ...
                fetch(`${this.apiPath}rand_textures/?id=${t}-${s}`).then((res => res.json())).then((res => {
                    this.loadModel(t, res.textures.id, "我的新衣服好看嘛？")
                }))
            }
        }
